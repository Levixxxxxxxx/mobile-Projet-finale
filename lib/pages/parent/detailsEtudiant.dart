...
class _DetailsEtudiantState extends State<DetailsEtudiant> {
  ...
  Future<void> absence() async {
    final response3 = await http.get(
      Uri.parse(
          'http://10.0.2.2:8000/api/trackin/list/absences/student/${widget.userId}'),
      headers: {
        'Authorization': 'Bearer ${widget.token}',
      },
    );
  ...
  Future<void> presence() async {
    final response3 = await http.get(
      Uri.parse(
          'http://10.0.2.2:8000/api/trackin/presence/student/year_segment/${widget.userId}/1'),
      headers: {
        'Authorization': 'Bearer ${widget.token}',
      },
    );
  ...
  Future<void> presence2() async {
    final response3 = await http.get(
      Uri.parse(
          'http://10.0.2.2:8000/api/trackin/presence/student/year_segment/${widget.userId}/2'),
      headers: {
        'Authorization': 'Bearer ${widget.token}',
      },
    );
  ...
  Future<void> presence3() async {
    final response3 = await http.get(
      Uri.parse(
          'http://10.0.2.2:8000/api/trackin/presence/student/year_segment/${widget.userId}/3'),
      headers: {
        'Authorization': 'Bearer ${widget.token}',
      },
    );
                  ...
                  Row(
                    ...
                    children: [
                      GestureDetector(
                        ...
                        child: Card(
                          color: selectedFeature == "timetable"
                    ...
                    selectedDay != null
                        ? ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              final filteredSeances = timetableData?["data"]
                                      ["seances"]
                                  ?.where((seance) {
                                final date = DateTime.parse(seance["date"]);
                                return _dayToString(date.weekday)
